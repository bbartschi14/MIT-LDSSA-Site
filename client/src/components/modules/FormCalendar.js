...
class FormCalendar extends Component {
  ...
  getDayElement = (day) => {
    ...
    let click;
    if (!day.isCurrentMonth || this.state.dayjs(day.date).isBefore(this.state.today)) {
      classList += " Calendar-notCurrent"
    } else if (this.props.datesTaken?.includes(day.date)) {
      classList += " Calendar-unavailable"
    } else if (day.date == this.props.selectedDay) {
      classList += " Calendar-selected"
    } else {
      classList += " Calendar-available"
      click = () => this.props.onDaySelect(day.date)
    }

    

    return (<li className={classList} onClick={click} key={`day_${day.date}`}><span >{day.dayOfMonth}</span></li>);
  }
